...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        g = {i: G[i] for i in fringe}
        curNode = list(g.keys())[list(g.values()).index(min(g.values()))]  # initialize the current node.
        curGscore = G[curNode]  # initialize the G score of current node
        ...
        if curNode == goal:
            ...
            for one_neigh in get_neighbors(curNode):
                if one_neigh not in expandedNodes:
                    cst = move_cost(one_neigh, barrier)
                    if one_neigh not in fringe:
                        fringe.add(one_neigh)
                        G[one_neigh] = G[curNode] + cst
                        parent[one_neigh] = curNode
                    else:
                        if G[curNode] + cst < G[one_neigh]:
                            G[one_neigh] = G[curNode] + cst
                            parent[one_neigh] = curNode
...
def aStarSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        f = {i: F[i] for i in fringe}
        curNode = list(f.keys())[list(f.values()).index(min(f.values()))]  # initialize the current node.
        curFscore = F[curNode]  # initialize the F score of current node
            ...
            for one_neigh in get_neighbors(curNode):
                if one_neigh not in expandedNodes:
                    ...
                    if one_neigh not in fringe:
                        fringe.add(one_neigh)
                        G[one_neigh] = G[curNode] + cst
                        F[one_neigh] = heuristic(one_neigh,goal) + G[curNode] + cst
                        parent[one_neigh] = curNode
                    else:
                        if G[curNode] + cst < G[one_neigh]:
                            G[one_neigh] = G[curNode] + cst
